  ...
  <section>
    ...
    <div v-else class="application-grid">
      <article v-for="application in applications" :key="application.id" class="application-card">
        <div><span class="status" :class="`status--${application.status}`">{{ application.status }}</span><h2>{{ application.vendor_name }}</h2><p>{{ application.description }}</p></div>
        <dl>
          ...
          <div><dt>Hours</dt><dd>{{ application.opening_hours }}</dd></div>
...
async function review(application, action) {
  updatingId.value = application.id; message.value = ''; error.value = ''
  try {
