<template>
  <section>
    <div class="page-heading"><div><span class="eyebrow">Student</span><h1>Notifications</h1><p>Kitchen updates for your orders.</p></div><button class="button button--small" :disabled="loading" @click="fetchNotifications">Refresh Notifications</button></div>
...
let pollTimer
async function fetchNotifications({quiet=false}={}){if(!quiet)loading.value=true;error.value='';try{const{data}=await api.get('/notifications');notifications.value=data.notifications??[]}catch(requestError){error.value=requestError.response?.data?.error||'Please check the API and try again.'}finally{loading.value=false}}
...
function startPolling(){if(pollTimer)return;pollTimer=setInterval(()=>fetchNotifications({quiet:true}),10000)}
onMounted(async()=>{await fetchNotifications();startPolling()})
