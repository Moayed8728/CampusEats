<template>
  <section class="auth-page">
    ...
    <form class="auth-card" @submit.prevent="submit"><div><p class="step-label">It takes a minute</p><h2>Create account</h2></div><label>Full name<input v-model.trim="form.name" type="text" autocomplete="name" placeholder="Your name" required></label><label>Email<input v-model.trim="form.email" type="email" autocomplete="email" placeholder="you@campus.edu" required></label><label>Password<input v-model="form.password" type="password" autocomplete="new-password" minlength="8" placeholder="At least 8 characters" required></label><label>Account type<select v-model="form.role"><option value="student">Student</option><option value="vendor">Vendor</option><option value="admin">Admin</option></select></label><p v-if="error" class="form-error">{{ error }}</p><button class="button auth-button" :disabled="loading">{{ loading ? 'Creating account…' : 'Create account' }} <span v-if="!loading">→</span></button><p class="auth-switch">Already have an account? <RouterLink to="/login">Sign in</RouterLink></p></form>
  </section>
</template>
...
const auth=useAuthStore();const router=useRouter();const form=reactive({name:'',email:'',password:'',role:'student'});const loading=ref(false);const error=ref('')
