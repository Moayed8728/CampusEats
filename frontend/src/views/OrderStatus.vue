<template>
  <section>
    ...
    <template v-else-if="order">
      ...
      <p class="tracking-note">{{ trackingNote }}</p>
      <div class="order-layout">
        ...
        <aside class="receipt-card">
          ...
          <div class="payment-details">
            <div><span>Payment method</span><strong>{{ paymentLabel(order.payment_method) }}</strong></div>
            <div><span>Payment status</span><strong>{{ title(order.payment_status || 'pending') }}</strong></div>
            <div v-if="order.payment_reference"><span>Reference</span><strong>{{ order.payment_reference }}</strong></div>
          </div>
...
const trackingNote = computed(() => 'Tracking updates automatically every 5 seconds.')
...
function paymentLabel(value = '') { return value === 'stripe' ? 'Stripe' : 'Payment' }
