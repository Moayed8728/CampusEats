<template>
  <section>
    <div class="page-heading"><div><span class="eyebrow">Vendor access</span><h1>Apply as Vendor</h1><p>Tell the admin team about the campus kitchen you want to run.</p></div></div>
    ...
    <div v-else-if="application?.status === 'pending'" class="status-card">
      <span class="status status--pending">Pending</span><h2>Your vendor application is currently under review.</h2><p>We’ll show the result here after an admin reviews {{ application.vendor_name }}.</p>
    </div>

    <div v-else-if="application?.status === 'approved'" class="status-card">
      <span class="status status--approved">Approved</span><h2>Your vendor application has been approved.</h2><p>You can now manage your vendor dashboard.</p><RouterLink class="button" to="/vendor/dashboard">Open Vendor Dashboard</RouterLink>
    </div>

    <template v-else>
      <div v-if="application?.status === 'rejected'" class="status-card status-card--soft"><span class="status status--rejected">Rejected</span><h2>Your previous application was rejected. You may submit a new application.</h2></div>
...
function looksLikeWords(value) { return /[a-z0-9]/i.test(value) && /[a-z]{2,}/i.test(value) && !/^[^a-z0-9]+$/i.test(value) }
function isRealisticOpeningHours(value) {
  ...
  return matches.every((match) => { const hour = Number(match[1]); const minute = match[2] ? Number(match[2]) : 0; return hour >= 1 && hour <= 12 && minute >= 0 && minute <= 59 })
}
...
async function fetchApplication() {
  loadingStatus.value = true; statusError.value = ''
  try {
    const { data } = await api.get('/vendor-applications/me')
    application.value = data.application
    if (data.role && auth.user) { auth.user.role = data.role; localStorage.setItem('user', JSON.stringify(auth.user)) }
  } catch (requestError) {
...
async function submit() {
  ...
  try {
    ...
    form.vendor_name = ''; form.description = ''; form.location = ''; form.opening_hours = ''
  } catch (requestError) {
